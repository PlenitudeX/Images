...
class simple_image_download:
    ...
    def download(self, keywords, limit, directoryname, extensions={'.jpg', '.png', '.ico', '.gif', '.jpeg'}):
        keyword_to_search = [str(item).strip() for item in keywords.split(',')]
        ...
        while i < len(keyword_to_search):
             #keyword_to_search[i]
            self._create_directories(main_directory, directoryname)
            url = 'https://www.google.com/search?q=' + quote(
            ...
            while j < limit:
                ...
                path = main_directory + directoryname
